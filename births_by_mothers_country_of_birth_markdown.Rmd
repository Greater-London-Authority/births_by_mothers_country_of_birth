...
author: "David Kingman, Senior Research and Statistical Analyst (Demography)"
date: "18/06/2020"
output:
...
# TODO change datastore page address to be births by mother's country of birth?
...
https://github.com/David-Kingman/births_by_parents_country_of_birth
...
Since 2001, the number of live births being recorded in London has changed from `r london_births_2001` to `r london_births_latest_year` births per year. The proportion of births which were to mothers who had been born outside the UK has changed from `r london_overseas_perc_2001` in 2001 to `r london_overseas_perc_latest_year` in the most recent year.

In `r latest_year`, the region of origin which supplied the largest number of births to non-UK-born mothers in London was `r current_top_region_name` with `r current_top_region_number`, followed by the `r current_second_region_name` which provided `r current_second_region_number`. The region of origin which has seen the largest change since 2001 is the `r region_biggest_change_name`, which went from `r region_biggest_change_2001` to `r region_biggest_change_latest_year` live births per year.   
...
most_foreign_born_region_perc<-
  ...
  mutate(overseas_perc=percent(overseas_perc)) %>% 
  slice_head()
...
london_total_table<-
  ...
  select(Year=year,
         ...
         `Pre-2004 EU countries`:`Rest of the world`) %>% 
  mutate_if(is.numeric, funs(perc=./`Total Births`)) %>% 
...
iwalk(borough_births_data_latest_year[5:13],
    ~ borough_births_data_latest_year %>%
      ...
      labs(title=paste0("Proportion of all live births that were to mothers born in\n", .y, ", ", latest_year),
           caption = "\nChart: GLA City Intelligence\nSource: ONS Births by parents' country of births statistics\nN.B. Data for City of London has been combined with Hackney")+
      ggsave(paste0("figures/maps/", .y, "_", latest_year, ".jpeg"), width=8, height=8, units="in"))
      # ggsave(paste0("figures/maps/", .y, "_", latest_year, ".jpeg"), type="cairo"))
